...
function chainsawxiv_dd4e_dataPreLoad(opts){
...
function chainsawxiv_dd4e_dataPostLoad(opts){

	// Store global context for search by class name
	chainsawxiv_dd4e_context = document.getElementById(opts['containerId']);

	// Set additional options
	opts['imagePath'] = 'http://omnichron.net/external/op/dst_exalted/dynamic_sheets/exalted/0.1/images/';
	opts['context'] = document.getElementById(opts['containerId']);
	opts['debugThreshold'] = -1;
	
	//Convert interface elements
	chainsawxiv_dd4e_convertLists(opts);
	chainsawxiv_dd4e_convertPips(opts);
	chainsawxiv_dd4e_convertChecks(opts);
	chainsawxiv_dd4e_convertAreas(opts);
	chainsawxiv_dd4e_convertLinks(opts);

}

// Called immediately before the sheet's data is saved
function chainsawxiv_dd4e_dataPreSave(opts) {

	// Set additional options
	opts['context'] = document.getElementById(opts['containerId']);

	chainsawxiv_dd4e_unconvertPips(opts);
	chainsawxiv_dd4e_unconvertLinks(opts);
	chainsawxiv_dd4e_unconvertChecks(opts);	
	chainsawxiv_dd4e_unconvertAreas(opts);
...
function chainsawxiv_dd4e_pips(oElement,opts){
...
function chainsawxiv_dd4e_convertPips(opts){
	...
	for (var i = 0; i < aSpans.length; i++){
		if (aSpans[i].className.match(/pips/)){
			pTemp = chainsawxiv_dd4e_pips(aSpans[i],opts);
			pTemp.showPips();
		}
	}
...
function chainsawxiv_dd4e_unconvertPips(opts) {
...
function chainsawxiv_dd4e_list(oElement,opts){
	...
	// Parses the number of columsn the list has from its class name
...
function chainsawxiv_dd4e_convertLists(opts){
	...
	for (var i = 0; i < aDivs.length; i++){
		if (aDivs[i].className.match(/list/)){
			...
			lTemp = chainsawxiv_dd4e_list(aDivs[i],opts);		
...
// Core Textarea Class
function chainsawxiv_dd4e_area(oElement,opts){
...
// Converts all properly classed divs in the context to lists
function chainsawxiv_dd4e_convertAreas(opts){
	...
	for (var i = 0; i < aSpans.length; i++){
		if (aSpans[i].className.match(/area/)){
			...
			// Convert each element to a full featured list object
			taTemp = chainsawxiv_dd4e_area(aSpans[i],opts);
			taTemp.activate();
...
function chainsawxiv_dd4e_unconvertAreas(opts){
...
// Applies a bunch of pips functionality to the specified element
function chainsawxiv_dd4e_check(oElement,opts){
...
// Converts all properly classed spans in the context to pips elements
function chainsawxiv_dd4e_convertChecks(opts){
	...
	for (var i = 0; i < aSpans.length; i++){
		if (aSpans[i].className.match(/check/)){
			pTemp = chainsawxiv_dd4e_check(aSpans[i],opts);
			pTemp.showCheck();
		}
	}
...
// Converts all the pips in the context back to thier basic values for saving
function chainsawxiv_dd4e_unconvertChecks(opts) {
...
function chainsawxiv_dd4e_convertLinks(opts){
...
function chainsawxiv_dd4e_unconvertLinks(opts){
...
function chainsawxiv_dd4e_getElementsByClassName(sClassName,sElementType){

	// Provide default element type
	if (!sElementType) sElementType = 'div';

	var aList = new Array();
	var aDivs = chainsawxiv_dd4e_context.getElementsByTagName(sElementType);
	for (var i = 0; i < aDivs.length; i++){
...
function chainsawxiv_dd4e_tabClick(oTab,sPageClass){
	
	// Abort if already on the selected tab
	var oPage = chainsawxiv_dd4e_getElementsByClassName('page_' + sPageClass)[0];
	if (oPage.style.display != 'none') return;
	
	// Set the length of the shorter page to match the longer page
	var oPages = chainsawxiv_dd4e_getElementsByClassName('main');
	var iHeight = 0;
	...
	if (sPageClass == 'sheet'){
		oTab.parentNode.style.backgroundImage = "url(http://omnichron.net/external/op/dst_exalted/dynamic_sheets/exalted/0.1/images/brass-tab-left.png)";
		chainsawxiv_dd4e_getElementsByClassName('page_sheet')[0].style.display = 'block';
		chainsawxiv_dd4e_getElementsByClassName('page_bio')[0].style.display = 'none';
	}
	else{
		oTab.parentNode.style.backgroundImage = "url(http://omnichron.net/external/op/dst_exalted/dynamic_sheets/exalted/0.1/images/brass-tab-right.png)";
		chainsawxiv_dd4e_getElementsByClassName('page_bio')[0].style.display = 'block';
		chainsawxiv_dd4e_getElementsByClassName('page_sheet')[0].style.display = 'none';	
	}
